...
export function FiliaisForm({ mainBranchDefaults }: FiliaisFormProps) {
  ...
  const {
    ...
    defaultValues: {
      branches: [
        {
          name: mainBranchDefaults?.name ?? '',
          phone: mainBranchDefaults?.phone || undefined,
        },
      ],
    },
  })
      ...
      <div className="bg-background rounded-xl border p-6 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            A primeira unidade abaixo é a própria empresa que você acabou de cadastrar (a
            matriz) — já preenchemos o nome pra você, mas pode ajustar. Só adicione outras
            linhas se você atender clientes em mais de um endereço físico.
          </p>
        </div>
          ...
          {fields.map((field, i) => (
            <div key={field.id}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-medium">
                      {i === 0 ? 'Matriz (sua empresa)' : `Filial ${i + 1}`}
                    </h3>
                  </div>
                  {i > 0 && (
                    <Button
                ...
                <InputField
                  label={i === 0 ? 'Nome da matriz *' : 'Nome da filial *'}
